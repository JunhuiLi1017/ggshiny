...
if (!exists("import_data")) import_data <- TRUE
if (!exists("show_data")) show_data <- TRUE
if (!exists("controls")) controls <- c("options", "filters", "geoms", "theme", "axes", "labs", "code")

ui <- navbarPage(
  title = "VolcanoProfiler: Interactive Volcano Plot for KRAS Gene Sets",
  id = "vp_nav",
  collapsible = TRUE,
  header = tags$head(
    tags$style(HTML("
        ...
        /* esquisse-like header and container */
        .esquisse-container { padding: 10px 15px; }
        .esquisse-header { padding: 8px 12px; border-bottom: 1px solid #e9ecef; margin-bottom: 10px; }
        .esquisse-header .pull-left { float: left; }
        .esquisse-header .pull-right { float: right; }
        .esquisse-sidebar { min-height: 700px; }
        .esquisse-main { min-height: 700px; }
        .clearfix { clear: both; }
      "))
  ),
  ...
  tabPanel(
    "Volcano Plot",
    div(
      class = "esquisse-container tab-panel",
      div(
        class = "esquisse-header",
        div(class = "pull-left", h4("Volcano Plot", style = "margin: 0; color: #2c3e50;")),
        div(
          class = "pull-right",
          if ("code" %in% controls) {
            tagList(
              downloadButton("download_plot", "Download PNG"),
              downloadButton("download_plot_html", "Download HTML")
            )
          }
        ),
        div(class = "clearfix")
      ...
      fluidRow(
        column(
                 width = 3,
          class = "sidebar-panel esquisse-sidebar",
        ...
        column(
                 width = 9,
          class = "main-panel esquisse-main",
