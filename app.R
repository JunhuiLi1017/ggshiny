...
# https://github.com/JoachimGoedhart/ggPlotteR
# https://github.com/JoachimGoedhart/ggPlotteR/blob/main/app.R
...
load_dataset <- function(dataset_name = NULL) {
  if (is.null(dataset_name)) {
    # Use the first dataset from Base R as default
    dataset_name <- available_datasets[["Base R"]][[1]]
  }
  
  if (grepl("::", dataset_name)) {
    # Handle package::dataset format
    parts <- strsplit(dataset_name, "::")[[1]]
    package <- parts[1]
    dataset <- parts[2]
    data(list = dataset, package = package, envir = .GlobalEnv)
    return(get(dataset))
  } else {
    # Handle base R datasets
    data(list = dataset_name, envir = .GlobalEnv)
    return(get(dataset_name))
  }
}

# Use the first available dataset as default
df <- load_dataset()

# Convert character columns to factors for categorical plotting
...
layer_controls_ui <- function(id_suffix, df, x_variables, y_variables) {
  div(
    ...
    conditionalPanel(
      ...
      # Add collapsible Color & Fill section
      hr(),
      checkboxInput(paste0("show_color_fill_", id_suffix), "Color & Fill", value = TRUE),
      conditionalPanel(
        condition = paste0("input.show_color_fill_", id_suffix, " == true"),
        uiOutput(paste0("color_var_ui_", id_suffix)),
        uiOutput(paste0("fill_var_ui_", id_suffix)),
        uiOutput(paste0("group_var_ui_", id_suffix))
      ),
      hr(),
      checkboxInput(paste0("show_legend_", id_suffix), "Legend", value = TRUE),
      conditionalPanel(
        condition = paste0("input.show_legend_", id_suffix, " == true"),
        uiOutput(paste0("legend_title_ui_", id_suffix)),
        uiOutput(paste0("legend_position_ui_", id_suffix)),
        uiOutput(paste0("legend_background_fill_ui_", id_suffix)),
        uiOutput(paste0("legend_text_size_ui_", id_suffix))
      )
    ),
...
ui <- fluidPage(
  useShinyjs(),
  titlePanel("Interactive Plotting with ggplot2 and Plotly"),
  sidebarLayout(
    sidebarPanel(
      id = "controls_panel",
      tabsetPanel(
        ...
        tabPanel("Layer 1",
                 layer_controls_ui("1", df, x_variables = colnames(df), y_variables = colnames(df))
        ),
        uiOutput("dynamic_layer_tabs"),
        tabPanel("Layer + / -",
                 h3("Manage Plot Layers"),
                 checkboxInput("show_layer_management", "Layer Management", value = TRUE),
                 conditionalPanel(
                   condition = "input.show_layer_management == true",
        ...
        tabPanel("Faceting",
                 ...
                 checkboxInput("show_faceting", "Faceting Options", value = TRUE),
                 conditionalPanel(
                   condition = "input.show_faceting == true",
        ...
        tabPanel("Statistics",
                 ...
                 checkboxInput("show_statistics", "Statistics Options", value = TRUE),
                 conditionalPanel(
        ...
        tabPanel("Plot Theme",
                 ...
                 checkboxInput("show_theme", "Theme Options", value = TRUE),
                 conditionalPanel(
                   condition = "input.show_theme == true",
    ...
    mainPanel(
      ...
      fluidRow(
        column(4,
               selectInput("dataset_package", "Select Package:",
                          choices = names(available_datasets),
                          selected = "Base R")
        ),
        column(4,
               selectInput("dataset_name", "Select Dataset:",
                          choices = available_datasets[["Base R"]],
                          selected = "mtcars")
        ),
        column(4,
               actionButton("load_dataset", "Load Dataset", style = "margin-top: 25px;")
        )
      ),
      # Variable information in 2x2 grid
      fluidRow(
        column(12,
               ...
               fluidRow(
                 column(6,
                        h5("Convert to Factor"),
                        uiOutput("to_factor_ui")
                 ),
                 column(6,
                        h5("Convert to Numeric"),
                        uiOutput("to_numeric_ui")
                 )
               )
        )
      ),
...
server <- function(input, output, session) {
  # Reactive value for the current dataset
  rv <- reactiveValues(
    num_layers = 1,
  ...
  # Reactive expressions for variable types
  factor_vars <- reactive({
    names(rv$current_df)[sapply(rv$current_df, is.factor)]
  })
  
  numeric_vars <- reactive({
    names(rv$current_df)[sapply(rv$current_df, is.numeric)]
  })
  
  # Render factor variables UI
      ...
      tagList(
        lapply(vars, function(var) {
          div(
            ...
            strong(var),
            br(),
            paste("Levels:", paste(levels(rv$current_df[[var]]), collapse = ", "))
          )
        })
      )
    } else {
      ...
      tagList(
        lapply(vars, function(var) {
          div(
            ...
            strong(var),
            br(),
            paste("Range:", round(min(rv$current_df[[var]], na.rm = TRUE), 2), "to", 
                  round(max(rv$current_df[[var]], na.rm = TRUE), 2))
          )
        })
      )
    } else {
  ...
  # Render convert to factor UI
  output$to_factor_ui <- renderUI({
    vars <- numeric_vars()
    if (length(vars) > 0) {
      tagList(
        selectInput("var_to_factor", "Select variable to convert:",
                   choices = vars),
        actionButton("convert_to_factor", "Convert to Factor")
      )
    } else {
      "No numeric variables to convert"
    }
  })
  
  # Render convert to numeric UI
  output$to_numeric_ui <- renderUI({
    vars <- factor_vars()
    if (length(vars) > 0) {
      tagList(
        selectInput("var_to_numeric", "Select variable to convert:",
                   choices = vars),
        actionButton("convert_to_numeric", "Convert to Numeric")
      )
    } else {
      "No factor variables to convert"
    }
  })
  
  # Handle convert to factor
  observeEvent(input$convert_to_factor, {
    req(input$var_to_factor)
    tryCatch({
      rv$current_df[[input$var_to_factor]] <- as.factor(rv$current_df[[input$var_to_factor]])
      rv$warnings <- character(0)
    }, error = function(e) {
      rv$warnings <- c(rv$warnings, paste("Error converting to factor:", e$message))
    })
  })
  
  # Handle convert to numeric
  observeEvent(input$convert_to_numeric, {
    req(input$var_to_numeric)
    tryCatch({
      rv$current_df[[input$var_to_numeric]] <- as.numeric(as.character(rv$current_df[[input$var_to_numeric]]))
      rv$warnings <- character(0)
    }, error = function(e) {
      rv$warnings <- c(rv$warnings, paste("Error converting to numeric:", e$message))
    })
  })
  
  # Add observer for y variable changes to update y-label
  observe({
    lapply(1:rv$num_layers, function(i) {
      id_suffix <- as.character(i)
      y_var <- input[[paste0("y_var_", id_suffix)]]
      if (!is.null(y_var) && y_var != "" && y_var != "None") {
        updateTextInput(session, paste0("y_label_", id_suffix), value = y_var)
      }
    })
  })
  
  # Update dataset choices when package changes
  observeEvent(input$dataset_package, {
    updateSelectInput(session, "dataset_name",
                     choices = available_datasets[[input$dataset_package]],
                     selected = available_datasets[[input$dataset_package]][1])
  })
  
  # Load dataset when button is clicked
  observeEvent(input$load_dataset, {
    tryCatch({
      new_df <- load_dataset(input$dataset_name)
      
      # Convert character columns to factors
      char_cols <- sapply(new_df, is.character)
      if (any(char_cols)) {
        new_df[char_cols] <- lapply(new_df[char_cols], as.factor)
      }
      
      # Update the current dataset
      rv$current_df <- new_df
      ...
      updateSelectInput(session, "x_var_1", selected = names(new_df)[1])
      updateSelectInput(session, "y_var_1", selected = names(new_df)[2])
      ...
      rv$warnings <- c(rv$warnings, paste("Error loading dataset:", e$message))
    })
  })
  
  # Render variable information table
  output$variable_info <- renderTable({
    ...
    var_info <- data.frame(
      ...
      Levels = sapply(df, function(x) {
        ...
        else if (is.numeric(x)) paste("Range:", round(min(x, na.rm = TRUE), 2), "to", round(max(x, na.rm = TRUE), 2))
        else ""
      })
    )
    var_info
  ...
  theme_parameters <- list(
    text = list(
      text_size = list(type = "numeric", default = 11, label = "Text Size"),
    ...
    plot = list(
      ...
      plot_margin_top = list(type = "numeric", default = 5.5, label = "Plot Margin Top (pt)"),
      plot_margin_right = list(type = "numeric", default = 5.5, label = "Plot Margin Right (pt)"),
      plot_margin_bottom = list(type = "numeric", default = 5.5, label = "Plot Margin Bottom (pt)"),
      plot_margin_left = list(type = "numeric", default = 5.5, label = "Plot Margin Left (pt)")
    ),
    legend = list(
      ...
      legend_text_size = list(type = "numeric", default = 9, label = "Legend Text Size")
    )
  )
  ...
  observeEvent(input$reset_to_default, {
    ...
    updateSelectInput(session, "linetype_val_1", value = default_values$linetype_val)
    ...
    updateCheckboxInput(session, "show_layer_management", value = default_values$show_layer_management)
    updateCheckboxInput(session, "show_faceting", value = default_values$show_faceting)
    updateCheckboxInput(session, "show_statistics", value = default_values$show_statistics)
    updateCheckboxInput(session, "show_theme", value = default_values$show_theme)
  ...
  observeEvent(input$add_layer_btn, {
    ...
    appendTab(inputId = "plot_settings_tabs",
              tabPanel(paste("Layer", new_layer_id, "Settings"),
                       layer_controls_ui(as.character(new_layer_id), df, x_variables = c(numeric_cols(), categorical_cols()), y_variables = c(numeric_cols(), categorical_cols()))
              ))
  ...
  observe({
    lapply(1:rv$num_layers, function(i) {
      ...
      output[[paste0("y_label_ui_", id_suffix)]] <- renderUI({
        ...
        default_y_label <- if (geom %in% c("histogram", "density", "freqpoly") || is.null(current_y_var) || current_y_var == "") "Count" else current_y_var
  ...
  generate_geom_layer <- function(layer_idx, input_obj, df_data) {
    ...
    # Ensure x variable is factor for boxplot and violin
    if (geom_type %in% c("boxplot", "violin") && !is.null(x_var_input)) {
      if (!is.factor(df_data[[x_var_input]])) {
        df_data[[x_var_input]] <- as.factor(df_data[[x_var_input]])
      }
    }
    ...
    if (!geom_type %in% c("hline", "vline", "abline")) {
      if (!is.null(x_var)) aes_list$x <- x_var
      if (!is.null(y_var)) aes_list$y <- y_var
    }
    
    color_res <- get_aesthetic_value_simplified(input_obj, id_suffix, "color_var")
    if (!is.null(color_res$aesthetic)) {
      color_var <- as.character(color_res$aesthetic)
      if (color_var %in% names(df_data) && !is.factor(df_data[[color_var]])) {
        df_data[[color_var]] <- as.factor(df_data[[color_var]])
      }
      aes_list$color <- color_res$aesthetic
    }
    
    fill_res <- get_aesthetic_value_simplified(input_obj, id_suffix, "fill_var")
    if (!is.null(fill_res$aesthetic)) {
      fill_var <- as.character(fill_res$aesthetic)
      if (fill_var %in% names(df_data) && !is.factor(df_data[[fill_var]])) {
        df_data[[fill_var]] <- as.factor(df_data[[fill_var]])
      }
      aes_list$fill <- fill_res$aesthetic
    }
    ...
    if (!is.null(shape_res$aesthetic)) {
      shape_var <- as.character(shape_res$aesthetic)
      if (shape_var %in% names(df_data) && !is.factor(df_data[[shape_var]])) {
        df_data[[shape_var]] <- as.factor(df_data[[shape_var]])
      }
      aes_list$shape <- shape_res$aesthetic
    }
    ...
    if (geom_type %in% c("line", "smooth", "abline", "hline", "vline", "freqpoly", "function", "path", "segment", "curve", "spoke", "ribbon")) {
      ...
      if (!is.null(linetype_res$aesthetic)) {
        linetype_var <- as.character(linetype_res$aesthetic)
        if (linetype_var %in% names(df_data) && !is.factor(df_data[[linetype_var]])) {
          df_data[[linetype_var]] <- as.factor(df_data[[linetype_var]])
        }
        aes_list$linetype <- linetype_res$aesthetic
      }
    }
    ...
    if (!is.null(group_res$aesthetic)) {
      group_var <- as.character(group_res$aesthetic)
      if (group_var %in% names(df_data) && !is.factor(df_data[[group_var]])) {
        df_data[[group_var]] <- as.factor(df_data[[group_var]])
      }
    ...
    # Add scale_discrete for factor aesthetics
    scale_args <- list()
    if (!is.null(color_res$aesthetic) && is.factor(df_data[[as.character(color_res$aesthetic)]])) {
      scale_args$color <- scale_color_discrete()
    }
    if (!is.null(fill_res$aesthetic) && is.factor(df_data[[as.character(fill_res$aesthetic)]])) {
      scale_args$fill <- scale_fill_discrete()
    }
    if (!is.null(shape_res$aesthetic) && is.factor(df_data[[as.character(shape_res$aesthetic)]])) {
      scale_args$shape <- scale_shape_discrete()
    }
    if (!is.null(linetype_res) && !is.null(linetype_res$aesthetic) && is.factor(df_data[[as.character(linetype_res$aesthetic)]])) {
      scale_args$linetype <- scale_linetype_discrete()
    }
    
    geom_layer <- tryCatch({
      layer <- switch(geom_type,
      ...
      # Add scale layers for factor aesthetics
      if (length(scale_args) > 0) {
        for (scale in scale_args) {
          layer <- layer + scale
        }
      }
  ...
  # Add observer for theme changes to update theme parameters
  observeEvent(input$plot_theme, {
    ...
    if (!is.null(theme_obj$text$size)) {
      updateNumericInput(session, "theme_text_size", value = theme_obj$text$size)
    }
    if (!is.null(theme_obj$text$colour)) {
      updateColourInput(session, "theme_text_color", value = theme_obj$text$colour)
    }
    
    # Update panel parameters
    if (!is.null(theme_obj$panel.background$fill)) {
      updateColourInput(session, "theme_panel_background_fill", value = theme_obj$panel.background$fill)
    }
    if (!is.null(theme_obj$panel.grid.major$colour)) {
      updateColourInput(session, "theme_panel_grid_major_color", value = theme_obj$panel.grid.major$colour)
    }
    if (!is.null(theme_obj$panel.grid.major$size)) {
      updateNumericInput(session, "theme_panel_grid_major_size", value = theme_obj$panel.grid.major$size)
    }
    if (!is.null(theme_obj$panel.grid.minor$colour)) {
      updateColourInput(session, "theme_panel_grid_minor_color", value = theme_obj$panel.grid.minor$colour)
    }
    if (!is.null(theme_obj$panel.grid.minor$size)) {
      updateNumericInput(session, "theme_panel_grid_minor_size", value = theme_obj$panel.grid.minor$size)
    }
    
    # Update plot parameters
    if (!is.null(theme_obj$plot.background$fill)) {
      updateColourInput(session, "theme_plot_background_fill", value = theme_obj$plot.background$fill)
    }
    if (!is.null(theme_obj$plot.margin)) {
      updateNumericInput(session, "theme_plot_margin_top", value = theme_obj$plot.margin[1])
      updateNumericInput(session, "theme_plot_margin_right", value = theme_obj$plot.margin[2])
      updateNumericInput(session, "theme_plot_margin_bottom", value = theme_obj$plot.margin[3])
      updateNumericInput(session, "theme_plot_margin_left", value = theme_obj$plot.margin[4])
    }
    
    # Update legend parameters
    if (!is.null(theme_obj$legend.position)) {
      updateSelectInput(session, "theme_legend_position", selected = theme_obj$legend.position)
    }
    if (!is.null(theme_obj$legend.background$fill)) {
      updateColourInput(session, "theme_legend_background_fill", value = theme_obj$legend.background$fill)
    }
    if (!is.null(theme_obj$legend.text$size)) {
      updateNumericInput(session, "theme_legend_text_size", value = theme_obj$legend.text$size)
    }
  })
}
